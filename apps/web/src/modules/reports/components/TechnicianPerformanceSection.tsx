...
export const TechnicianPerformanceSection: React.FC<TechnicianPerformanceSectionProps> = ({ technicianData }) => {
  const breakdown = technicianData?.technicianBreakdown ?? [];
  const technicians: TechnicianReportItem[] = breakdown.length
    ? breakdown.map((t: any, idx: number) => ({
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-base font-bold text-slate-900">Technician Performance</h2>
          <p className="text-xs text-slate-500">Field workforce execution SLA, turnaround time, and job completion rates</p>
        </div>
      </div>
              ...
              {technicians.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-8 text-center text-slate-400 font-medium">
                    No field technician assignments recorded in this period.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={tech.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-4 font-semibold text-slate-900">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full bg-blue-50 text-blue-700 font-bold text-[10px] flex items-center justify-center border border-blue-200">
